...
print calc_argmaxs
print np.sum(calc_hits_argmaxs, axis = 0)
...
plt.hist(preds_argmaxs, histtype='step', bins=51,  color = 'red', label = r"Classification: $\alpha^{CP}_{max}$")

#plt.step(np.arange(0,51),np.sum(calc_hits_argmaxs, axis = 0),  color = 'black', linestyle='--', label = "Generated")
#plt.step(np.arange(0,51),np.sum(preds_hits_argmaxs,axis = 0),  color = 'red', label = r"Classification: $\alpha^{CP}_{max}$")
