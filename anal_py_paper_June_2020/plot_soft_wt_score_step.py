...
for i in range(3,52,2):
    ...
    calc = np.load("/net/people/plgerichter/HiggsCP_2019/CPmix-kacper/HiggsCP/temp_results/nn_rhorho_Variant-All_soft_weights_hits_c0s_Unweighted_False_NO_NUM_CLASSES_{0}_SIZE_1000_LAYERS_6/monit_npy/softmax_calc_w.npy".format(i))
    preds = np.load("/net/people/plgerichter/HiggsCP_2019/CPmix-kacper/HiggsCP/temp_results/nn_rhorho_Variant-All_soft_weights_hits_c0s_Unweighted_False_NO_NUM_CLASSES_{0}_SIZE_1000_LAYERS_6/monit_npy/softmax_preds_w.npy".format(i))
...
plt.semilogy(np.arange(3,52,2),score,'o', label=r'$l_2$ with $wt^{norm}$', color = "black")
