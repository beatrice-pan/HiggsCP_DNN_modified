...
for i in range(3,52,2):
    ...
    for j in range(5000):
        ...
        cs_calc[int((i-3)/2),j], _ = optimize.curve_fit(weight_fun, x2, calc[j]/sum(calc[j]), p0=[1, 1, 1])
        cs_pred[int((i-3)/2),j], _ = optimize.curve_fit(weight_fun, x2, preds[j]/sum(calc[j]), p0=[1, 1, 1])
