...
parser.add_argument("-o", "--output", dest="OUT", default=os.environ["A1RHO_DATA"])
parser.add_argument("-s", "--source", dest="SOURCE", default=os.environ["DATA_SOURCE"])

args = parser.parse_args()

prefix = args.SOURCE
a1rho_data_path = args.OUT

for letter in ["a", "b", "c", "d", "e", "f", "g", "h", "i", "k"][:args.DATASETS]:
    for mname in ["scalar", "pseudoscalar"]:
        name = "pythia.H.a1rho.1M.%s.%s.outTUPLE_labFrame" % (letter, mname)
        if os.path.exists(os.path.join(a1rho_data_path,  name)):
            print name, " is already downloaded! Skipping."
            continue
        link = prefix + name
        print link
        os.system("wget -P " + a1rho_data_path + " " + link)
